...
const CONFIG = {
    ...
    // 不再使用 5MB 的人为总容量限制；IndexedDB 的实际配额由浏览器决定。
    // 单文件仅保留一个 2GB 的安全阈值，避免移动端误选超大文件。
...
function getVideoUrl(v) {
    if (!v) return '';
    if (v.blob) {
        // IndexedDB 返回的 Blob/File 在部分移动浏览器中可能存在跨 realm / instanceof 判断问题。
        // 只要它具备 Blob 的基本接口，就重新构造一个标准 Blob。
        try {
            const blob = v.blob instanceof Blob ? v.blob : new Blob([v.blob], { type: v.type || 'video/mp4' });
            if (!state.objectUrls.has(v.id)) state.objectUrls.set(v.id, URL.createObjectURL(blob));
            return state.objectUrls.get(v.id);
        } catch (e) {
            console.error('创建视频 URL 失败:', e);
        }
    }
    if (v.buffer instanceof ArrayBuffer) {
        try {
            const blob = new Blob([v.buffer], { type: v.type || 'video/mp4' });
            if (!state.objectUrls.has(v.id)) state.objectUrls.set(v.id, URL.createObjectURL(blob));
            return state.objectUrls.get(v.id);
        } catch (e) {
            console.error('从 ArrayBuffer 创建视频 URL 失败:', e);
        }
    }
...
function handleFileSelect(file) {
    ...
    if (file.size > CONFIG.MAX_FILE_SIZE) return showToast('单个视频超过 2GB，移动端不建议直接导入', 'error');
...
async function checkStorageBeforeUpload(fileSize) {
    ...
    try {
        ...
        // 预留少量空间给数据库事务及其他站点数据，避免刚好写满。
        if (fileSize + CONFIG.STORAGE_RESERVE > available) {
            showToast(`可用空间不足：当前约剩 ${formatSize(Math.max(available, 0))}`, 'error');
            return false;
        }
    } catch (e) {
        console.warn('无法读取浏览器存储配额，将继续尝试上传：', e);
    }
    return true;
}
async function doUpload() {
    ...
    btn.disabled = true; btn.textContent = '检查空间…';
    try {
        if (!(await checkStorageBeforeUpload(file.size))) return;

        // 用户主动点击上传时尝试申请持久化存储，浏览器可能拒绝，但不影响正常使用。
        if (navigator.storage?.persist) {
            try { await navigator.storage.persist(); } catch {}
        }

        btn.textContent = '保存中…';
        // 直接保存 Blob，不经过 Base64，不会产生约 33% 的 Base64 体积膨胀。
        const safeBlob = file.slice(0, file.size, file.type || 'video/mp4');
        const video = { id: Date.now().toString(36) + Math.random().toString(36).slice(2,8), name: file.name, category: cat, blob: safeBlob, size: file.size, type: file.type || 'video/mp4', createdAt: Date.now() };
        ...
        console.error(e);
        const message = e.name === 'QuotaExceededError' ? '浏览器存储空间不足，请删除旧视频或清理站点数据' : (e.message || '存储失败');
...
async function exportBackup() {
    ...
    try {
        ...
        for (const v of state.data.videos) {
            let data;
            if (v.blob) {
                const exportBlob = v.blob instanceof Blob ? v.blob : new Blob([v.blob], { type: v.type || 'video/mp4' });
                data = await blobToDataURL(exportBlob);
            } else if (v.buffer instanceof ArrayBuffer) {
                data = await blobToDataURL(new Blob([v.buffer], { type: v.type || 'video/mp4' }));
            } else {
                data = v.data;
            }
