...
viewMode:'grid',
gridCardMode:localStorage.getItem('videoLib_gridCardMode_v1')||'large',
currentTag:'全部',
...
border:1px solid rgba(214,139,164,.28);
border-radius:10px;
}
...
background:var(--pink);
color:#fff;
box-shadow:0 2px 6px rgba(183,107,134,.22);
}
...
/* 选中标签后的内容不是标签分组，而是完整视频网格 */
.tag-filtered-view .video-grid{
width:100%;
...
// 主页网格提供两种卡片密度；标签页不再承担“小窗/大框”两套职责。
if(state.viewMode==='grid'){
const cardModeSwitch=document.createElement('div');
cardModeSwitch.className='video-card-mode-switch';
[['large','大框'],['small','小窗']].forEach(([mode,label])=>{
const btn=document.createElement('button');
btn.className='video-card-mode-btn'+(state.gridCardMode===mode?' active':'');
btn.type='button';
btn.textContent=label;
btn.addEventListener('click',()=>{
state.gridCardMode=mode;
localStorage.setItem('videoLib_gridCardMode_v1',mode);
renderFilterBar();
renderVideos();
});
cardModeSwitch.appendChild(btn);
});
bar.appendChild(cardModeSwitch);
}
...
if(state.viewMode==='tags'&&tags.length){
...
// 标签页的“全部”显示标签分组；选中具体标签后，
// 仍停留在标签页，但内容区域直接使用标准视频网格卡片。
if(state.currentTag==='全部'){
renderTagView(vs,box);
}else{
box.classList.add('tag-filtered-view');
renderGrid(vs,box,true);
}
...
<div
class="quick-tag-list"
id="editVideoQuickTagList">
</div>
...
const quickTagList=
modal.querySelector(
'#editVideoQuickTagList'
);

const renderQuickEditTags=()=>{
if(!quickTagList)return;
...
function renderGrid(vs,box,forceLarge=false){

const grid=document.createElement('div');

grid.className='video-grid'+(!forceLarge && state.gridCardMode==='small'?' compact':'');
...
chip.type='button';
chip.className='quick-tag-btn';
chip.textContent=`#${tag}`;

chip.addEventListener('click',()=>{
tags.push(tag);
renderBatchTags();
renderQuickBatchTags();
});
...
// 修改：HTML 中已内置上传标签区，直接给页面上的按钮绑定事件
// （原来绑定的是动态生成的 uploadTagAddBtn，页面按钮从未被绑定）
...
select.innerHTML=state.cats.filter(c=>c!=='全部').map(c=>
'<option value="'+escapeHtml(c)+'">'+escapeHtml(c)+'</option>'
).join('');
if(current&&state.cats.includes(current))select.value=current;
}
...
document.getElementById('catManageBtn')?.addEventListener('click',openCategoryManager);
}
...
const openBtn=document.getElementById('uploadBtnMain');
...
document.getElementById('playerDownloadBtn')?.addEventListener('click',()=>{
 const v=state.data.videos.find(x=>x.id===state.currentPlayerId);
 if(v)downloadVideo(v);
});
...
const open=document.getElementById('addCatBtn');
const input=document.getElementById('newCatName');
const confirmBtn=document.getElementById('addCatConfirmBtn');
...
const add=()=>{
 ...
 if(name==='全部'||name==='未分类')return toast('该分类名称不可用','error');
 if(state.cats.includes(name))return toast('分类已存在','error');
 state.cats.push(name);
 if(!saveCats())return;
 closeModal('catModal');
 renderCats();renderFilterBar();populateUploadCategories();
 toast('分类已添加');
};
